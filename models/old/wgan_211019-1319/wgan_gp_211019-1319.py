...
class GANMonitor(keras.callbacks.Callback):
    def __init__(self, real, feature_names=None, log_dir=None, frequency = 10, sclr=None) -> None:
        ...
        self.sclr = sclr
        if sclr is not None:
            self.real = self.sclr.inverse_transform(self.real)
        if feature_names:
...
if __name__ == "__main__":
    epochs, batch_size, freq = 200000, 10000, 10
    ...
    train = np.load("data/processed/X_minmax.npy")
    # test = np.load("data/processed/X_minmax.npy")
    ...
    monitor = GANMonitor(train[:100000], feature_names, frequency=freq, sclr=sclr)
    callbacks= [ monitor ]
