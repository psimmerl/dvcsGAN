...
class WGAN(keras.models.Model):
    ...
    def compile(self, d_opt, g_opt):#, d_loss_fn, g_loss_fn):
        ...
        def discriminator_loss(real, fake):
            return tf.reduce_mean(tf.random.normal((tf.shape(fake)[0], 1), 1, 0.05) * fake) -\
                   tf.reduce_mean(tf.random.normal((tf.shape(real)[0], 1), 1, 0.05) * real)
...
if __name__ == "__main__":
    epochs, batch_size, freq = 250, 800, 10
    ...
    monitor = GANMonitor(train, feature_names, frequency=freq, sclr=sclr)
    callbacks= [ monitor ]
    ...
    wgan = WGAN()
    wgan.compile(d_opt=
                # RMSprop(learning_rate=0.000005),
                Adam(learning_rate=0.0001, beta_1=0.5, beta_2=0.9),
                g_opt=
                # RMSprop(learning_rate=0.000005),
                Adam(learning_rate=0.0001, beta_1=0.5, beta_2=0.9)
                )
