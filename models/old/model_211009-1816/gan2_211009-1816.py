...
class GAN(keras.models.Model):
    def __init__(self, discriminator=None, generator=None, latent_dim=256, gen_dim=19) -> None:
    ...
    def train_step(self, real):
        ...
        y_generated = tf.ones((batch_size, 1)) +  0.05 * tf.random.uniform((batch_size, 1))
        y_real = tf.zeros((batch_size, 1)) +  0.05 * tf.random.uniform((batch_size, 1))
...
if __name__ == "__main__":
    ...
    gan = GAN()
    gan.compile(d_opt=Adam(learning_rate=0.0002, beta_1=0.5, beta_2=0.9),
                g_opt=Adam(learning_rate=0.0002, beta_1=0.5, beta_2=0.9),
