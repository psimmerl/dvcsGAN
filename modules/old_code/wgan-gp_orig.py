...
class WGAN(keras.models.Model):
    ...
    def train_step(self, real):
        ...
        g_gradient = tape.gradient(g_loss, self.gen.trainable_variables)
        self.g_opt.apply_gradients(zip(g_gradient, self.gen.trainable_variables))
...
class GANMonitor(keras.callbacks.Callback):
    def __init__(self, real, feature_names=None, log_dir=None, frequency = 10) -> None:
        ...
        for i in range(real.shape[1]):
                ax = self.axs[i]
                ax.hist(self.real[:,i],histtype='step',bins=self.bins,label='real')
                ax.grid(which='both')
                ax.set_xlim([-1,1])
                # ax.set_yscale('log')
...
if __name__ == "__main__":
    epochs, batch_size, freq = 10000, 500, 5
    ...
    if dirty_version_preserve:
        os.system(f"cp ./wgan-gp.py {monitor.log_dir}/wgan-gp_{monitor.log_dir.split('_')[-1]}.py")

    wgan = WGAN()
    wgan.compile(d_opt=#RMSprop(learning_rate=0.000005),
                Adam(learning_rate=0.0002, beta_1=0.5, beta_2=0.9),
                g_opt=#RMSprop(learning_rate=0.000005),
                Adam(learning_rate=0.0002, beta_1=0.5, beta_2=0.9)
                )
